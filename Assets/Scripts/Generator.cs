...
public abstract class Generator : MonoBehaviour
{
    ...
    [Header("Rivers")]
    [SerializeField]
    protected bool EnableRivers = false;
    [SerializeField]
    ...
    protected virtual void Generate()
    {
        ...
        BiomeMapRenderer.materials[0].mainTexture = TextureGenerator.GetBiomeMapTexture(Width, Height, Tiles, ColdestValue, ColderValue, ColdValue, Outline);
    }

    private IEnumerator genAndSave()
    {
        System.Random rnd = new System.Random(DateTime.UtcNow.Millisecond);
        bool saveData = Directory.Exists(path);
        ...
        for (int i = 0; i < qty; i++)
        {
            ...
            Debug.Log(DateTime.Now.ToString("h:mm tt") + " Starting generation of " + i);
            yield return new WaitForFixedUpdate();

            Generate();
            Texture2D map = (Texture2D)BiomeMapRenderer.materials[0].mainTexture;

            if (saveData)
            {
                safePath = path + rnd.Next() + ".png";
                while (File.Exists(safePath)) safePath = path + rnd.Next() + ".png";
            }

            Debug.Log(DateTime.Now.ToString("h:mm tt") + " generated " + i);
            yield return new WaitForFixedUpdate();
            yield return new WaitForFixedUpdate();

            if (saveData)
            {
                byte[] data = map.EncodeToPNG();
                using (FileStream file = new FileStream(safePath, FileMode.CreateNew)) file.Write(data, 0, data.Length);
                Debug.Log(DateTime.Now.ToString("h:mm tt") + " written file " + i + " to " + safePath);
            }
            yield return new WaitForSeconds(delay);
        }

        Debug.Log(DateTime.Now.ToString("h:mm tt") + " Done");
    }
